...
public class SafetyNetStage3Test extends StageTest {
  ...
  @DynamicTest
  CheckResult test4() {
    // Test if the commit count in the 'main' branch is 4
  ...
  @DynamicTest
  CheckResult test5() {
    // Test if the commit messages in the 'main' branch are correct
  ...
  @DynamicTest
  CheckResult test6() {
    // Test if the 'feature/math' branch has been deleted
    String deletedBranch = "feature/math";
    try (Git git = new Git(repository)) {
      List<String> branches = git.branchList().call().stream()
              .map(ref -> ref.getName().replace("refs/heads/", ""))
              .toList();
